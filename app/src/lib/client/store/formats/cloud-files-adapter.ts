...
import { projectRelativeCloudPaths } from '$lib/client/sync/cloud-paths';
...
export { projectCloudRootPath, projectRelativeCloudPaths } from '$lib/client/sync/cloud-paths';
export type { ProjectCloudPaths } from '$lib/client/sync/cloud-paths';

export async function serializeCloudFile(file: CloudFile): Promise<string> {
	const canonical = cloudFileToCanonicalPayload(file);
	return serializeCanonicalDocument(canonical.format, canonical.payload);
}

function cloudFileToCanonicalPayload(file: CloudFile): { format: string; payload: JsonObject } {
	if (isProjectCloudFile(file)) {
		return { format: PROJECT_MANIFEST_FORMAT, payload: projectCloudFileToPayload(file) };
	}
	if (isProjectTranscriptionCloudFile(file)) {
		return {
			format: PROJECT_TRANSCRIPTION_FORMAT,
			payload: projectTranscriptionCloudFileToPayload(file),
		};
	}
	if (isCollationCloudFile(file)) {
		return { format: COLLATION_FORMAT, payload: collationCloudFileToPayload(file) };
	}
	if (isHistoryCloudFile(file)) {
		if (file.entity_type === 'project-transcription') {
			return {
				format: TRANSCRIPTION_CHECKPOINT_FORMAT,
				payload: transcriptionHistoryCloudFileToPayload(file),
			};
		}
		return {
			format: COLLATION_CHECKPOINT_FORMAT,
			payload: collationHistoryCloudFileToPayload(file),
		};
	}
	return { format: TOMBSTONE_FORMAT, payload: tombstoneCloudFileToPayload(file) };
}
...
export async function parseProjectTranscriptionCloudFile(
	...
	return result.ok
		? { ok: true, value: projectTranscriptionPayloadToCloudFile(result.payload) }
		: { ok: false, quarantine: result.quarantine };
}
...
export async function parseCollationCloudFile(
	...
	return result.ok
		? { ok: true, value: collationPayloadToCloudFile(result.payload) }
		: { ok: false, quarantine: result.quarantine };
}
