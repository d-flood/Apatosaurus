...
export async function listManifestSourcesForUrl(
	...
	const rows = await db
		...
		.selectAll()
		.where('transcription_id', '=', transcriptionId)
		.where('manifest_url', '=', manifestUrl)
		.execute();
	return rows.map(rowToManifestSource);
}
...
export async function ensureManifestSource(
	...
	const existing = await db
		.selectFrom('iiif_manifest_sources')
		.selectAll()
		.where('transcription_id', '=', input.transcriptionId)
		.where('manifest_url', '=', input.manifestUrl)
		.executeTakeFirst();
	...
	await db
		...
		.onConflict(oc =>
			oc.columns(['transcription_id', 'manifest_url']).doUpdateSet({
				label: input.label || input.manifestUrl,
				source_kind: input.sourceKind || 'external',
				default_canvas_id: input.defaultCanvasId || null,
				default_image_service_url: input.defaultImageServiceUrl || null,
...
export async function getManifestSource(
	db: DbExecutor,
	input: ManifestSourceIdInput
): Promise<ManifestSourceSummary | null> {
	const row = await db
		.selectFrom('iiif_manifest_sources')
		.selectAll()
		.where('transcription_id', '=', input.transcriptionId)
		.where('id', '=', input.manifestSourceId)
		.executeTakeFirst();
	return row ? rowToManifestSource(row) : null;
}

export async function listPageCanvasLinks(
	db: DbExecutor,
	transcriptionId: string
...
export async function upsertPageCanvasLink(
	...
	const existing = await db
		...
		.where('manifest_source_id', '=', input.manifestSourceId)
		.where('link_role', '=', input.linkRole || 'primary')
		.executeTakeFirst();
	const id = existing?.id || crypto.randomUUID();
	await db
		.insertInto('transcription_page_canvas_links')
		.values({ ...linkInputToRow(input, id, existing?.created_at || now, now) })
		.onConflict(oc =>
			oc
				.columns(['transcription_id', 'page_id', 'manifest_source_id', 'link_role'])
				.doUpdateSet({
...
export async function deletePageCanvasLink(
	db: DbExecutor,
	input: DeletePageCanvasLinkInput
): Promise<number> {
...
export async function deletePageCanvasLinksForPage(
	db: DbExecutor,
...
export async function deleteAllPageCanvasLinks(
	db: DbExecutor,
	transcriptionId: string
): Promise<number> {
	const result = await db
...
export async function deleteManifestSource(
	db: DbExecutor,
	input: ManifestSourceIdInput
): Promise<boolean> {
	const result = await db
...
export async function deleteManifestSourceLinks(
	db: DbExecutor,
	input: ManifestSourceIdInput
): Promise<number> {
	const result = await db
...
export async function findLinkedPageForCanvas(
	db: DbExecutor,
	input: FindLinkedPageForCanvasInput
...
export async function listCanvasAnnotations(
	db: DbExecutor,
	input: ListCanvasAnnotationsInput
	...
	const rows = await db
		...
		.selectAll()
		.where('transcription_id', '=', input.transcriptionId)
		.where('manifest_source_id', 'in', input.manifestSourceIds)
		.where('canvas_id', '=', input.canvasId)
		.execute();
	return rows.map(row =>
		rowToCanvasAnnotation(row, {
			mode: input.mode || 'full',
			previewLength: input.previewLength || 280,
		})
	);
}

export async function getCanvasAnnotation(
	db: DbExecutor,
	input: CanvasAnnotationIdInput
...
export async function upsertCanvasAnnotation(
	db: DbExecutor,
	input: UpsertCanvasAnnotationInput
): Promise<void> {
	const now = new Date().toISOString();
	const existing = await db
		.selectFrom('iiif_canvas_annotations')
		...
		.where('transcription_id', '=', input.transcriptionId)
		.where('manifest_source_id', '=', input.manifestSourceId)
		.where('annotation_id', '=', input.annotation.id)
		.executeTakeFirst();
	...
	await db
		...
		.values({
			...
			created_by:
				input.createdBy ||
				input.annotation.creator?.id ||
				input.annotation.creator?.name ||
				'',
			created_at: existing?.created_at || input.annotation.created || now,
			updated_at: now,
		})
		.onConflict(oc =>
			oc.columns(['transcription_id', 'manifest_source_id', 'annotation_id']).doUpdateSet({
				page_id: input.pageId,
				canvas_id: input.canvasId,
				target_json: JSON.stringify(input.annotation.target || { source: input.canvasId }),
				body_json: JSON.stringify(input.annotation.body || []),
...
export async function deleteCanvasAnnotation(
	db: DbExecutor,
	input: CanvasAnnotationIdInput
): Promise<void> {
	await db
...
function linkInputToRow(
	input: SavePageCanvasLinkInput,
	id: string,
	createdAt: string,
	updatedAt: string
): Selectable<TranscriptionPageCanvasLinks> {
	return {
		id,
		transcription_id: input.transcriptionId,
		page_id: input.pageId,
...
function rowToManifestSource(row: Selectable<IiifManifestSources>): ManifestSourceSummary {
	...
	return {
		...
		label: row.label,
		sourceKind: row.source_kind as 'external' | 'app',
...
function buildPreviewBodies(
	body: unknown,
	maxLength: number
): { bodies: W3CAnnotationBody[]; preview: string | null } {
	...
	const preview =
		normalized
			...
			.find(value => value.length > 0) || null;
	return {
		bodies: normalized.map(entry =>
			typeof entry.value === 'string'
				? { ...entry, value: truncateAnnotationText(entry.value, maxLength) }
				: { ...entry }
		),
		preview,
	};
