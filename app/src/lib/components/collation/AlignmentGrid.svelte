<script lang="ts">
	...
	let newPatternError = $derived(
		newPattern.trim() ? collationState.validateRegularizationPattern(newPattern) : null
	);
...
>
	...
	{/if}
	{#if collationState.isAlignmentStale}
		<div class="alert alert-warning mb-3 text-sm">
			Rules or collation settings changed after this alignment was run. Re-run needed before readings match the current preview.
		</div>
	{/if}
				...
				{:else}
					<div class="space-y-2">
						{#each collationState.rules as rule (rule.id)}
							...
							{@const changedEffects = collationState.getRuleEffects(rule.id).filter(effect => effect.changed)}
							<div
