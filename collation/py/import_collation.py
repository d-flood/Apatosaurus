...
from accounts.py.update_status import update_status, JobStatus
...
def tei_to_db(tei_file_name: str, section_id: int, user_pk: int):
    ...
    tei_string = s3.get_object(Bucket=settings.AWS_STORAGE_BUCKET_NAME, Key=tei_file_name)['Body'].read().decode('utf-8')
    job_pk = JobStatus.objects.create(user_id=user_pk, name=f'Import TEI Collation {models.Section.objects.get(pk=section_id).name}', message='Started').pk
    try:
