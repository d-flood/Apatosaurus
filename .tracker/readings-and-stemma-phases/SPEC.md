...
**Editorial decisions become the authority, and the reading proposal becomes recomputable.** The reading proposal — which readings exist, which look like subreadings, provisional order, likely reading types — is a pure function of the alignment and the collation settings, derived on read and never stored as truth. Editorial decisions are a sparse record of what a scholar explicitly established. What the phases display is the proposal with decisions applied. Nothing re-derives on write, so no decision can be eaten. Reshaping the alignment by hand — merging, splitting, shifting a token — stops destroying editorial work at every unit it did not touch, because a decision is keyed by the alignment column its unit covers rather than by that unit's position. A decision naming a reading, or a unit, that no longer exists becomes an orphaned decision surfaced for resolution rather than silently dropped.

Surviving a **re-collation** is a separate and larger problem, and this epic does not solve it. The aligner assigns a fresh id to every column on every run (`collation-adapter.ts`, `makeId()`), so nothing derived from a column id survives re-collation and every decision orphans. Reattaching them needs a content-derived anchor — base-text token offset, or a key built from the readings themselves — and that anchor is deliberately not designed here: the wrong one silently reattaches a decision to the wrong unit, which is the exact failure this epic exists to remove. Today re-collation discards readings and stemma edges outright, so the epic still strictly improves on the status quo; it just does not close this gap. Closing it needs its own ticket and its own design.
