# Issue 10: Folder Import and Unified Ingestion

Architecture reference: `../architecture.md` sections 3 (decision 7), 8

## What to build

On Chromium, a user can pick a local directory containing a project folder and import it through the same staged-ingestion pipeline issue 09 built for zips. `project-restore.ts` is refactored onto the store so that zip import, folder import, and sync-pull share one validated ingestion path — no parallel restore implementations remain.

When the picked folder is a project folder from another machine with the **same project id** but unknown revisions, that is the sync case — direct the user to connect it as a sync target (issue 07 flow) instead of importing. A **different project id** is the import case and proceeds through ingestion.

## Where to start

- The staged-ingestion entry point from issue 09 (built to accept a readable file tree).
- `app/src/lib/client/sync/project-restore.ts` — `importCloudProject()` and `pullLinkedProjectUpdates()`; this issue retires their bespoke ingestion in favor of the shared path.
- `app/src/lib/client/sync/providers/local-folder-provider.ts` and `local-folder-handles.ts` — reading a picked directory.
- `app/src/lib/client/sync/sync-manager.ts` — the Issue 07 pull path; its post-pull validation should end up calling the same ingestion primitive.
...
- Collision handling, path hygiene, and reporting follow the issue 09 contract unchanged.
- Folder import is feature-gated on `showDirectoryPicker` support; non-Chromium sees the zip path.

## Out of scope

- New sync semantics or scheduling changes (issue 07 owns those).
- Copy-with-lineage flows (issue 11).
- Onboarding/capability messaging beyond the feature gate (issues 12, 16).
...
- [ ] Corrupt-file and traversal behavior matches issue 09 (shared tests exercise the folder source).
