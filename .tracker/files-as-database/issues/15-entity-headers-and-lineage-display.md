# Issue 15: Entity Headers and Lineage Display
...
2. **Lineage surfacing**: a copied transcription shows "copied from `<project>` @ `<short-hash>`" with a staleness indicator when the source has newer commits. The data already exists from issues 04/11 (`origin_*` fields, revision hashes); this is display work.
...
- Commit state: working-file presence / dirty status from the issue 05 load paths; committed head from the manifest/index listings.
...
- Staleness display is informational; refreshing remains the explicit issue 11 flow (link to it, do not trigger it).
...
- Navigation/tab structure (issue 14).
- Refresh-from-source behavior (issue 11).
- Any editor content or selection behavior (issues 19-20).
