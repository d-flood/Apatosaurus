...
| Date | Note |
| --- | --- |
| 2026-07-13 | Review remediation completed. `mirrorProjectFiles()` now owns production tombstone ordering/application, per-path staged canonical validation, partial-success fingerprinting, and primary-derived TEI handling. Pulls and conflicts regenerate TEI from the accepted primary on each side rather than trusting remote TEI bytes. Permission loss enters a visible reconnect-required state, stops polling, and resumes after project-scoped re-grant; the global navbar folder connector and its `cloud_connections` state were removed. Added two-store push/pull/divergence/no-op/cache-rebuild coverage plus tombstone interruption, mixed valid/corrupt/conflicting/remote-only, TEI, and permission lifecycle tests. Verification passed: focused sync/store suites (120 tests), database generation/check, type checking, and full unit/browser suite (489 tests). |
