...
Ticket 05 is reopened because the main commit sequences are file-first, but active bypasses and working-file lifecycle errors violate the global write-path contract.

### Required fixes

- After successful transcription commit, delete the committed working file as the collation path does, or record enough base/head metadata to prove it remains uncommitted. Load/rebuild may prefer working state only when demonstrably different from the committed primary.
- Make transcription and collation creation failure-safe. Do not leave a visible index seed if history, primary, manifest, or initial commit creation fails. Create files before index publication or clean up seed rows at every failed boundary.
- Route project metadata, transcription metadata, IIIF source/link/annotation changes, project fork/copy, and refresh-from-source through canonical file-first operations. None may update SQLite and write files later.
- Make refresh-from-source persist replacement checkpoint/primary/manifest before changing index content/projections. Preserve dirty state as a canonical checkpoint first, as ticket 11 requires.
- Return structured non-blocking TEI warnings from commit/create operations and show them in the UI. Logging alone does not satisfy "log and surface."
- Keep the documented deletion order that preserves the old primary until the tombstone manifest is durable, but update the ticket sequence text to match it. A primary-delete failure must be visible as recoverable state.

### Required tests

- Commit a transcription, write/pull a newer primary, and prove an old working file cannot mask it.
- Inject every creation-step failure and assert no durable index-only entity remains.
- Inject refresh failures around each canonical write and assert old state remains readable or new state is fully recoverable.
- Mutate metadata and IIIF state, delete/rebuild the index, and assert survival.
