# Ticket 21: Data-Safety Invariant Test Suite
...
1. **Index deletion is lossless** — extend the ticket 06 delete-the-index test to include sync fingerprints and quarantine reports.
...
6. **Crash-ordering leaves readable state** — promote the ticket 05 crash tests into the suite, run against both entity types.
7. **Working-state crash survival** — simulate a kill mid-autosave-write; reload recovers the previous working file.
8. **Persistence requested and surfaced** — unit-level assertion (ticket 12 behavior); manual checklist for real browser behavior.
9. **Zip round-trip equivalence** — promote the ticket 09 round-trip test; run in the CI browser matrix where possible.

Defects surfaced by the suite are fixed here or filed as new tickets in this tracker — not left floating.

## Where to start

- New suite home: `app/src/lib/client/store/invariants.spec.ts` (browser mode where OPFS is needed).
- Donor tests to promote: ticket 06's delete-the-index browser-worker invariant spec; ticket 05's crash-ordering tests in `app/src/lib/client/db/repositories/*-files.spec.ts`; ticket 09's round-trip test.
...
- Playwright end-to-end scenarios and CI wiring (ticket 22).
