# Ticket 13: Backup-Health Panel and Install Nudge
...
2. **PWA install nudge**: a considered, dismissible install prompt shown once a user has real data (installed PWAs get persistence and better permission retention on Chromium). Uses install state from ticket 12's `capabilities.ts`.

## Where to start

- `app/src/lib/client/sync/backup-health.ts` and `backup-status.ts` — existing logic to absorb; keep their tests' intent.
- `app/src/lib/components/projects/ProjectBackupPanel.svelte` — the Folder Sync UI from ticket 07; the health panel extends or sits beside it.
- Last-synced comes from ticket 07's fingerprint/sync state; last exported from wherever ticket 08 recorded it (check TRACKER notes); last committed from checkpoint listings.
- `capabilities.ts` (ticket 12) for install state and `beforeinstallprompt` handling.
...
- Persistence banner and storage estimate (ticket 12).
- Navigation placement changes (ticket 14) — build the panel where `ProjectBackupPanel` lives today; ticket 14/15 may re-home it.
- Onboarding content (ticket 16).
...
- 2026-07-07: Implementation stopped before code changes because the contract expects last-exported to come from timestamp data recorded by ticket 08, and also says not to add health persistence beyond already-recorded timestamps. The current code only keeps `exportedAt` in live component state after `exportProjectZip()` / `exportAllProjectsZip()` returns; no persisted per-project or account export timestamp exists in the store or app settings. Needs a human decision before implementation: add export timestamp persistence, relax the health contract, or split that prerequisite into a separate ticket.
- 2026-07-07: Human validation resolved this blocker by approving option 1: ticket 13 may persist last-export timestamps in app-local settings/store data as rebuildable health metadata, not irreplaceable user data.
