...
from cbgm import forms, models, tasks
    ...
    if request.user.is_authenticated:
        return render(
            ...
            "cbgm/index.html",
            {
                ...
                "entire_dbs": models.Cbgm_Db.objects.filter(
                    user=request.user, amount=2, active=True
                ),
                "section_dbs": models.Cbgm_Db.objects.filter(
                    user=request.user, amount=1, active=True
                ),
                "verse_dbs": models.Cbgm_Db.objects.filter(
                    user=request.user, amount=0, active=True
                ),
            },
        )
    return render(
...
def send_section_form(request: HttpRequest, corpus_pk: int, corpus_type: int):
    if corpus_type == 0:
        ...
        return render(
            ...
            {"message": "Invalid corpus type", "timeout": 4},
        )
    ...
    return render(
        ...
        "scraps/quick_message.html",
        {
            "message": 'Collation Export to the CBGM Enqueued. You can track this under "Background Tasks" in your profile. Note that large collations will usually take 1 to 2 second per witness including correctors.',
            "timeout": 4,
        },
    )
...
def edit_db(request: HttpRequest, db_pk: int):
    ...
    if request.method == "GET":
        ...
        return render(
            request,
            "cbgm/activated_db.html",
            {
                "db": db,
                "form": forms.UpdateCbgmDbForm(instance=db),
                "sorted_witnesses": witnesses,
                "sorted_app_labels": app_labels,
                "compare_wits_form": forms.CompareWitnessesForm(
                    all_witnesses=witness_options
                ),
                "find_relatives_form": forms.FindRelativesForm(
                    all_witnesses=witness_options, app_labels=app_choices
                ),
                "app_groups": app_groups,
                "optimize_substemma_form": forms.OptimizeSubstemmaForm(
                    all_witnesses=witness_options
                ),
        ...
        resp = render(
            request,
            "scraps/quick_message.html",
            {"message": "Database updated", "timeout": 4},
        )
        ...
        resp = render(
            request,
            "scraps/quick_message.html",
            {"message": "Database deleted", "timeout": 4},
        )
        resp["HX-Trigger"] = "refreshDbs"
        return resp
    ...
    return HttpResponse(
        render_block_to_string(
            ...
            "db_options",
            {
                "page": {"title": "Apatosaurus - open-cbgm", "active": "open-cbgm"},
                "entire_dbs": models.Cbgm_Db.objects.filter(
                    user=request.user, amount=2
                ),
                "section_dbs": models.Cbgm_Db.objects.filter(
                    user=request.user, amount=1
                ),
...
def compare_witnesses(request: HttpRequest, db_pk: int) -> HttpResponse:
    ...
    if form.is_valid():
        comparison = cbgm.compare_witnesses(
            db, form.cleaned_data["witness"], form.cleaned_data["comparative_witnesses"]
        )
...
def find_relatives(request: HttpRequest, db_pk: int) -> HttpResponse:
    ...
    form = forms.FindRelativesForm(
        request.POST, all_witnesses=all_witnesses, app_labels=app_labels
    )
    if form.is_valid():
        rdgs = request.POST.getlist("variation-unit-readings")
        relatives = cbgm.find_relatives(
            db, form.cleaned_data["witness"], form.cleaned_data["app_labels"], rdgs
        )
...
@login_required
@require_safe
def get_rdgs_for_app(
    request: HttpRequest, db_pk: int, variation_unit: str
) -> HttpResponse:
    db = models.Cbgm_Db.objects.get(pk=db_pk)
    cached_db = cbgm.get_cached_db(db).resolve().as_posix()
...
def optimize_substemma(request: HttpRequest, db_pk: int) -> HttpResponse:
    ...
    if form.is_valid():
        successful, substemma = cbgm.optimize_substemma(
            db=db,
            witness=form.cleaned_data["witness"],
...
def local_stemma(request: HttpRequest, db_pk: int, variation_unit: str) -> HttpResponse:
    ...
    if successful:
        return render(
            request, "cbgm/local_stemma.html", {"svg": svg, "app": variation_unit}
        )
    resp = HttpResponse(status=204)
    resp["HX-Trigger"] = (
        f'{{"showDialog": {{"title": "Error", "message": "No local stemma found for this variation unit."}}}}'
    )
    return resp
...
def get_svg_task_status(request: HttpRequest):
    ...
    if not job_id:
        resp = HttpResponse(status=204)
        resp["HX-Trigger"] = (
            """{"showDialog": {"title": "No Task", "message": "There is no task running or completed for this operation."}}"""
        )
        return resp
    ...
    if job.completed:
        if job.textual_flow:
            resp = render(
                request, "cbgm/textual_flow.html", {"result": json.loads(job.data)}
            )
        else:
        ...
        resp = HttpResponse(status=204)
        resp["HX-Trigger"] = (
            f'{{"showDialog": {{"title": "Error", "message": "{job.message}"}}}}'
        )
        return resp
    else:
        return HttpResponse(status=204)
