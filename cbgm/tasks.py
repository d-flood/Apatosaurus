...
def import_tei_batch_job(user_pk: int, section_pk: int, db_pk: int, corpus_type: int):
    ...
    response = client.submit_job(
        ...
        containerOverrides={
            'command': [
                'python', 'manage.py', 'populate_cbgm_db', str(user_pk), str(section_pk), str(db_pk), str(corpus_type)
            ]
        },
...
def global_stemma_batch_job(job_pk: int, db_pk: int, data: dict):
    ...
    response = client.submit_job(
        ...
        containerOverrides={
            'command': [
                'python', 'manage.py', 'print_global_stemma', str(job_pk), str(db_pk), json.dumps(data)
            ]
        },
...
def textual_flow_batch_job(job_pk: int, db_pk: int, data: dict):
    ...
    response = client.submit_job(
        ...
        containerOverrides={
            'command': [
                'python', 'manage.py', 'print_textual_flow', str(job_pk), str(db_pk), json.dumps(data)
            ]
        },
